...
(function(ext) {
  ...
  function processSysexMessage() {
    switch(storedInputData[0]) {
      ...
      case QUERY_FIRMWARE:
        ...
        break;
      case STRING_DATA:
         var storedStringData = new Uint8Array(MAX_DATA_BYTES);
         for (var i = 1; i < sysexBytesRead; i++) 
           storedStringData[i-1] = storedInputData[i];

         console.log(String.fromCharCode.apply(null, storedStringData));
         break;
    }
  }
  ...
  function audioStart() {
    var output = new Uint8Array([START_SYSEX, AUDIO_START, END_SYSEX]);
    device.send(output.buffer);
  }
    
  function audioStop() {
    var output = new Uint8Array([START_SYSEX, AUDIO_STOP, END_SYSEX]);
    device.send(output.buffer);
  }
    
  ext.audioStart = function() {
    audioStart();
  };
    
  ext.audioStop = function() {
    audioStop();
  };
  ...
  var descriptor = {
    blocks: blocks[lang],
    menus: menus[lang],
    url: 'http://neusoft.github.io/scratch-spresense-extension'
  };

  ScratchExtensions.register('SPRESENSE', descriptor, ext, {type:'serial'});
